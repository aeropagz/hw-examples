...
{   //struct myStruct received;
	int notCopied;
	int j;
	printk("write aufgerufen mit count  %d\n",count);
	printk("requesting  %d bytes\n",sizeof(strus[index]));
	notCopied=copy_from_user((void*)&strus[index],user,sizeof(strus[index]));
	
	 printk("Nachricht:%s i:%d  j:%d values:",strus[index].nachricht,strus[index].i,strus[index].j);
